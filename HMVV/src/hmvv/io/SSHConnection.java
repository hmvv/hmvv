...
public class SSHConnection {
	...
	public static int getFileSize(Pipeline pipeline) throws Exception {
		String command="";
		if  ( pipeline.getInstrumentID().equals("miseq")) {
			command = String.format("ls -l /home/%s/*_%s_*/Data/Intensities/BaseCalls/Alignment/%s_*.vcf | awk '{print $5}' ", pipeline.getInstrumentID(), pipeline.getRunID(), pipeline.getsampleName());
		} else if  ( pipeline.getInstrumentID().equals("nextseq")) {
			command = String.format("ls -l /home/%s/*_%s_*/out1/%s*_R1_001.fastq.gz | awk '{print $5}' ", pipeline.getInstrumentID(), pipeline.getRunID(), pipeline.getsampleName());
		}else if  ( pipeline.getInstrumentID().equals("proton")) {
			command = String.format("ls -l /home/%s/*%s/plugin_out/variantCaller_out*/%s/TSVC_variants.vcf | awk '{print $5}' ", pipeline.getInstrumentID(), pipeline.getRunID(), pipeline.getsampleName());
		}
