...
public class EnterCGPSample extends JDialog {
    ...
    private Assay cgpAssay = Assay.getAssay("CGP");
    private RunFolder runFolder = null;
    private ArrayList<String> samples = new ArrayList<String>();

    public EnterCGPSample(HMVVFrame parent, SampleListFrame sampleListFrame) {
        super(parent, "Enter CGP Sample", ModalityType.APPLICATION_MODAL);
    ...
    private void createComponents(){
        ...
        try{
            for(Instrument instrument : DatabaseCommands.getInstrumentsForAssay(cgpAssay)){
                instrumentComboBox.addItem(instrument);
            }
        }catch(Exception e){
            HMVVDefectReportFrame.showHMVVDefectReportFrame(EnterCGPSample.this, e, "Error getting Instruments from database");
            dispose();
            return;
        }

        assayTextField = new JTextField(cgpAssay.assayName); 
        ...
        enterSampleButton = new JButton("Enter CGP Samples");
                ...
                findRunThread = new Thread(new Runnable() {
                    ...
                    public void run() {
                        try {
                            ...
                            JOptionPane.showMessageDialog(EnterCGPSample.this, "Error finding run: " + e.getMessage());
                        }
                    }
                });
                findRunThread.start();
                    ...
                    enterSampleThread = new Thread(new Runnable() {
                        ...
                        public void run() {
                            if(runFolderTextField.getText().equals("")){
                                JOptionPane.showMessageDialog(EnterCGPSample.this, "Please select a run before entering samples.");
                                return;
                            }
                            if(sampleListTextArea.getText().equals("")){
                                JOptionPane.showMessageDialog(EnterCGPSample.this, "No samples found. Please select a valid run before entering samples.");
                                return;
                            }
                            ...
                            try {
                                ...
                                HMVVDefectReportFrame.showHMVVDefectReportFrame(EnterCGPSample.this, e, "Error entering sample data");
                                enterSampleButton.setText("Enter CGP Samples");
            ...
            if(samples.size() != 0){
                ...
                for(String currentSample : samples){
                    try{
                        ...
                        HMVVDefectReportFrame.showHMVVDefectReportFrame(EnterCGPSample.this, e);
                    }
                }
            }
        }finally {
    ...
    private Sample constructSampleFromTextFields(String currentSample) throws Exception{
        ...
        return new Sample(sampleID, cgpAssay, instrument, runFolder, "", "", "", "", "", "", "", runID, sampleName, "", "", runDate, "", "", "", enteredBy,analyzedBy);
    }
    ...
    private void findRun() throws Exception{
        ...
        SSHConnection.checkSampleSheetError(instrument, runFolder.runFolderName, cgpAssay.assayName); 
